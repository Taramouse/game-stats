          ...
          <v-container fluid grid-list-sm>
            <v-layout row>
              <v-flex xs12 class="pa-0 ma-0">
                <div>
                  <v-card-text class="primary display-3">{{profile.name}}
                  </v-card-text>
                  <v-card-text class="primary subheading">{{profile.description}}
                  </v-card-text>
                </div>
              </v-flex>
            </v-layout>
            <v-layout row wrap class="hidden-sm-and-down">
              <v-flex xs12>
                <v-card-text>
                  <v-data-table :headers="profile.headers" :items="data" hide-actions disable-initial-sort class="elevation-1" dark>
                  </v-data-table>
                ...
                <v-card-text>
                  <v-list>
                    <v-list-tile v-for="header in profile.headers" :key="header.value">
                      ...
                      <v-list-tile-content class="align-end">No data displayed during preview</v-list-tile-content>
                    </v-list-tile>
                  </v-list>
                </v-card-text>
              </v-flex>
